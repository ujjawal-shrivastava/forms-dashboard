...
export default function CheckboxField(props: any) {
    ...
    const handleDefaultOption = (index: number) => {
        setItem((old: any) => {
            old = { ...old }
            old.props.options[index].checked=!old.props.options[index].checked
            var arr = old.props.defaultValue.length?old.props.defaultValue.split(","):[]
            var idx = arr.indexOf(old.props.options[index].value)
            if(idx>-1){
                arr.splice(idx,1)
            }else{
                arr.push(old.props.options[index].value)
            }
            old.props.defaultValue= arr.toString()
            return old
        })
    }
                        ...
                        return (
                            <div key={index} style={{ margin: "1rem", background: "#dcdde1", padding: "1rem", borderRadius: "10px 10px 10px 10px" }}>
                                ...
                                <div className="buttons">
                                    ...
                                    <button className={`button is-small ${item.props.options[index].checked?"is-light":"is-dark"}`} onClick={() => { handleDefaultOption(index) }}>
                                    {item.props.options[index].checked?"Checked":"Default Check"}
                                        </button>
